...
```{r insallLatestversions, echo=FALSE, message=FALSE, eval=FALSE}
#library(devtools)
...
if(!exists("OUTPUTDIR")){
  OUTPUTDIR = "/scratch/wolski/specL/res.49530"
  WORKDIR = "/scratch/wolski/specL/temp.49530"
  FASTA_FILE <- "/usr/local/mascot/sequence/SwissProt/current/SwissProt_2016_02.fasta"
  MAX_IONS <- 6
  MIN_IONS <- 5
...
- Protein with most peptide assignments:
```{r}
max(tmp)
...
Number of annotated precursors is :
```{r} 
dim(pepProtMatrix)[1]
```
There are in total:
...
```{r}
dim(pepProtMatrix)[1] - sum(uniq == 1)
```
...
dim(pepProtMatrix)[1]
```
peptide precursors matched protein sequences assigned to 
```{r} 
dim(pepProtMatrix)[2]
``` 
unique protein identifies.

The minimal protein set explaining precursors has a size of 
```{r}
length(unique(unlist(protPepAssingments)))
``` 
proteins.
